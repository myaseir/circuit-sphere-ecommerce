...
const SingleItem = ({ item, removeItemFromCart }: { item: any, removeItemFromCart: any }) => {
  ...
  const imageUrl = 
    item.image && item.image.length > 0 
      ? (Array.isArray(item.image) ? item.image[0] : item.image) // Handle API data
      : (item.imgs?.thumbnails?.[0] || "/images/product/product-01.png"); // Handle old data or Fallback
