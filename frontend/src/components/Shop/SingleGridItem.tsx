...
const SingleGridItem = ({ item }: { item: Product }) => {
  ...
  const getImageUrl = () => {
    ...
    if (item.imgs?.previews && item.imgs.previews.length > 0) return item.imgs.previews[0];
    return placeholder;
  };
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[270px] mb-4 border border-gray-2">
        ...
        <Link href={`/shop/${item.id}`} className="block w-full h-full p-4">
