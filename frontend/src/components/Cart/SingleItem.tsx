...
import {
  removeItemFromCart,
  updateQuantity, // ✅ Use the correct action name
} from "@/redux/features/cart-slice";
import { Product } from "@/types/product";

// Define the item type including quantity
...
const SingleItem = ({ item }: { item: CartItem }) => {
  ...
  // ✅ 1. SAFE IMAGE HELPER (Prevents the crash)
  const getImageUrl = () => {
    // New Database Format (Array)
    ...
    // Old Template Format
    if (item.imgs?.thumbnails && item.imgs.thumbnails.length > 0) {
      return item.imgs.thumbnails[0];
    }
    // Fallback
  ...
  return (
    <div className="flex items-center border-t border-gray-3 py-5 px-7.5">
      <div className="min-w-[400px]">
        <div className="flex items-center justify-between gap-5">
          <div className="w-full flex items-center gap-5.5">
            {/* Image Container */}
            <div className="relative flex items-center justify-center rounded-[5px] bg-gray-2 max-w-[80px] w-full h-17.5 overflow-hidden border border-gray-200">
              <Image
                src={getImageUrl()} // ✅ Uses the safe helper
      ...
      <div className="min-w-[180px]">
        <p className="text-dark font-medium">${item.discountedPrice || item.price}</p>
      </div>
      ...
      <div className="min-w-[275px]">
        <div className="w-max flex items-center rounded-md border border-gray-3">
          <button
            ...
            className="flex items-center justify-center w-11.5 h-11.5 ease-out duration-200 hover:text-blue hover:bg-gray-100"
          >
          ...
          <span className="flex items-center justify-center w-16 h-11.5 border-x border-gray-3 font-semibold text-dark">
            {item.quantity}
          </span>
      ...
      <div className="min-w-[50px] flex justify-end">
        <button
          ...
          className="flex items-center justify-center rounded-lg w-9 h-9 bg-gray-100 border border-gray-200 text-dark hover:bg-red-50 hover:border-red-200 hover:text-red-500 transition-all"
        >
